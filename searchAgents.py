...
def cornersHeuristic(state, problem):
    ...
    pos = state[0]
    remained_conner = [corner for corner in corners if corner not in state[1]]

    while len(remained_conner) > 0:
        distance_array = [manhattanDistance(pos, corner) for corner in remained_conner]
        distance_min = min(distance_array)
        distance_index = distance_array.index(distance_min)
        pos = remained_conner[distance_index]
        remained_conner.pop(distance_index)
        result += distance_min
